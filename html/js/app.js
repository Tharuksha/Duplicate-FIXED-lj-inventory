            ...
            if (CanQuickMove()) {
                ...
                if (toAmount >= 0) {
                    if (
                     ...
                     else if ($fromInv.attr("data-inventory").search("Trunk-") !== -1) {
                        $.post("https://lj-inventory/UpdateTrunkData", JSON.stringify({}))
                    }
                    else if ($fromInv.attr("data-inventory").search("Stash-") !== -1) {
                        $.post("https://lj-inventory/UpdateStashData", JSON.stringify({}))
                    }
                    else if ($fromInv.attr("data-inventory").search("Glovebox-") !== -1) {
                        $.post("https://lj-inventory/UpdateGloveboxData", JSON.stringify({}))
                    }
                    else if ($fromInv.attr("data-inventory").search("Dropped-") !== -1) {
                        $.post("https://lj-inventory/UpdateDropData", JSON.stringify({}))
                    }
                }
            } else {
                InventoryError(fromInventory, fromSlot);
...
function swap($fromSlot, $toSlot, $fromInv, $toInv, $toAmount) {
    ...
    if (fromData !== undefined && fromData.amount >= $toAmount) {
        ...
        if ((toData != undefined || toData != null) && toData.type == "weapon") {
            $.post("https://lj-inventory/ResetWeaponSlot", JSON.stringify({
                slotToCheck: $toSlot,
                newSlot: $fromSlot
            }));
       }
